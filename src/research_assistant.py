...
class ResearchQuery(BaseModel):
    ...
    depth: str = Field(
        description="Research depth: quick, moderate, deep", default="moderate"
    )
    focus_areas: List[str] = Field(
        description="Specific areas to focus on", default_factory=list
    )
...
class ResearchResult(BaseModel):
    ...
    confidence_score: float = Field(
        description="Confidence in findings (0-1)", ge=0, le=1
    )
    model_used: str
    timestamp: str
...
class MultiModelResearchAssistant:
    ...
    async def research(
        self, query: ResearchQuery, model_name: Optional[str] = None
    ) -> ResearchResult:
...
async def demonstrate_research_assistant(models: Dict[str, BaseChatModel]):
    ...
    print(
        f"Research assistant initialized with models: {list(assistant.tool_models.keys())}\n"
    )
    ...
    if len(assistant.tool_models) > 1:
        ...
        try:
            ...
            for model_name, result in comparison_results.items():
                ...
                print(
                    f"- First finding: {result.key_findings[0] if result.key_findings else 'None'}"
                )

            # Find best result
            best_model = max(
                comparison_results.items(), key=lambda x: x[1].confidence_score
            )
            print(
